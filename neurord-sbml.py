...
def main(args):
    ...
    for child in ic_root:
        if child.tag == "ConcentrationSet":
            for child2 in child:
                concDict[child2.attrib['specieID']] = float(child2.attrib['value']) # Nanomolarity to Molarity
 ...
 #   compartment.setName(comp)
    ...
    units = {0:per_ms, 1:per_nM_per_ms, 2:per_nM2_per_ms, 3:per_nM3_per_ms, 4:per_nM4_per_ms}
        ...
        if child.tag == "Specie":
            ...
            for reactant in child:
                if reactant.tag == "Reactant":
                    ...
                    kin_law = '{}*(kon_{}*{} )'.format(comp, reacNum, '*'.join(reac_ord)) #'comp*(kon*E*S-koff*ES)' 
                    
                    model.addParameter('kon_{}'.format(reacNum), float(reactant.text), units=units[total_rord].id)
                    
                elif reactant.tag == 'reverseRate':
                    # local_params.update({'koff':float(reactant.text)})
                    prod_ord = [r + '^' + porder[r] if r in porder[r]!='1' else r for r in products]
                    ...
                    model.addParameter('koff_{}'.format(reacNum), float(reactant.text), units=units[total_pord].id)
                    
            # print(reactants, products, kin_law, local_params)
            model.addReaction(reactants, products, kin_law, local_params=local_params, rxn_id='r{}'.format(reacNum))
